...
for sensor in "${sensores[@]}"; do
    for nn in "${redeneural[@]}"; do
      echo ""
      echo "=== Processando: $cenario $nn $sensor  ==="
      outdir="output/${nn}/${sensor}/${cenario}"
      best_file="${outdir}/best_hyperparameters.json"
      summary_file="${outdir}/summary_metrics.csv"
      if [ -f "$best_file" ]; then
        if [ ! -f "$summary_file" ]; then
          echo "Treinando modelos finais: $cenario $nn $sensor "
          python final_training.py -scenario "$cenario" -position "$sensor" --nn "$nn" --num_models 30
          if [ $? -ne 0 ]; then
            echo "Erro no treinamento final. Pulando..."
            continue
          fi
          echo "Agregando métricas dos modelos finais..."
          python aggregate_metrics.py -scenario "$cenario" -position "$sensor" --nn "$nn"
          if [ $? -ne 0 ]; then
...
for cenario in "${cenarios[@]}"; do
  for sensor in "${sensores[@]}"; do
      for nn in "${redeneural[@]}"; do
        echo ""
        echo "=== Análises: $cenario $nn $sensor  ==="
        outdir="output/${nn}/${sensor}/${cenario}"
        summary_file="${outdir}/summary_metrics.csv"
        ...
        if [ -f "$summary_file" ]; then
          ...
          python permutation_importance.py -scenario "$cenario" -position "$sensor" --nn "$nn"
          # Learning Curve
          if [ ! -f "$lc_metrics_file" ]; then
            echo "Rodando curva de aprendizado (learning curve) para $cenario $nn $sensor "
            python learning_curve.py -scenario "$cenario" -position "$sensor" --nn "$nn"
          else
          ...
          if [ ! -f "$shap_file" ]; then
            ...
            python shap_importance.py -scenario "$cenario" -position "$sensor" --nn "$nn"
            if [ $? -eq 0 ]; then
              touch "$shap_file"
